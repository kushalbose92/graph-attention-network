...
model = GAT(data_obj.num_features, hidden_dim, data_obj.num_classes, dropout, alpha, nheads, device)
...
if use_saved_model == 'False':
    ...
    model_path = trainer.train(model, data_obj, adj_matrix, train_iter, opti, hidden_layers, device)
...
for i in range(test_iter):
    ...
    acc = eval.test(model, data_obj, adj_matrix, hidden_layers, model_path, device, is_validation = False)
    if acc > max_acc:
        max_acc = acc
    avg_acc += acc
